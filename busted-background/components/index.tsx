...
const MessageList: React.FunctionComponent = () => {
  const query = useQuery(() =>
    db.collection('messages').orderBy('timestamp', 'desc').limit(messageLimit)
  );
      ...
      .map((x) => {
        ...
        return (
          <li key={data.timestamp}>
            <strong style={{ color: getColorForUser(data.user.username) }}>
              {data.user.username}
            </strong>
            : {data.message}
          </li>
...
const TodoList: React.FunctionComponent = () => {
  ...
  if (query && metadata) {
    ...
    todos = docs.map((x) => {
      ...
      return (
        <li key={x.id}>
          <span style={{ marginLeft: indent * INDENT_SIZE_PX, marginRight: 6 }}>
            {data.completedAt != null ? FINISHED_EMOJI : NOT_FINISHED_EMOJI}
          </span>
          {data.description}
        </li>
      );
  ...
  if (bottomLink && bottomLink.exists) {
    bottomLinkMarkup = (
      <>
        ...
        <h2 style={{ marginLeft: 32, paddingLeft: 8, paddingRight: 8 }}>
          {bottomLink.data()!.text}
        </h2>
    ...
    <>
      ...
      <PageContainer>
        ...
        <div className="grid">
          <main>
            <h1>
              If you're seeing this then something bad happened!- {theWidth}
            </h1>
          </main>
